...
;RuntimeError("Send me the shields and ill fix")
;If Not(FileType("StagePacks\StagePack1.swarc")=1) Then RuntimeError("Game Archive was not found. Please make sure it is in the same directory as this .exe")
;If Not(FileType("StagePacks\StagePack2.swarc")=1) Then RuntimeError("Game Archive was not found. Please make sure it is in the same directory as this .exe")
Menu\Developer=1

CreateDir(GetEnv$("AppData")+"\Sonic World DX")
ResetOptions_Values()
...
Global LilFont=LoadFont("ChaoWorld/Impress.ttf",25,False,False,False)
...
Include "_SourceCode\Game\Game_Functions.bb"
Include "_SourceCode\Game\Game_Interface.bb"
Include "_SourceCode\Game\Game_Interface_ControlTips.bb"
...
; /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/
	...
	While(1)
		If (KeyHit(Key_Escape)) And Menu\Settings\Debug#=1 Then End ; Game_End()
