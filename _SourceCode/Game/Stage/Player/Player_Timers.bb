...
Global timervalue#=0
Global secs#=1000
Function TimerDone(p.tPlayer,o.tObject,timer)
	If (Not(timer>0)) Then Return True Else Return False
End Function
Function Player_HandleTimers(p.tPlayer)
	...
	If p\Action=ACTION_DRIFT Then
		If Input\Hold\Left Or Input\Hold\Right Then
			p\DriftTimer=p\DriftTimer+(timervalue#*1.65)
		Else
			p\DriftTimer=0
		EndIf
	EndIf
	
	If p\QuickstepTimer>0 Then 
		p\QuickstepTimer=p\QuickstepTimer-timervalue#
		For d.tDeltaTime = Each tDeltaTime
			Select p\QuickstepDir
				Case 0: TranslateEntity p\Objects\Entity,Cos(p\Animation\Direction#)*(p\QuickstepSpeed#*d\Delta),0,Sin(p\Animation\Direction#)*(p\QuickstepSpeed#*d\Delta)
				Case 1: TranslateEntity p\Objects\Entity,-Cos(p\Animation\Direction#)*(p\QuickstepSpeed#*d\Delta),0,-Sin(p\Animation\Direction#)*(p\QuickstepSpeed#*d\Delta)
			End Select		
		Next
	EndIf
	If p\BlazeHoverTimer>0 Then p\BlazeHoverTimer=p\BlazeHoverTimer-timervalue#
	p\AmySpinTimer=p\AmySpinTimer-timervalue#
	
	If p\LightAttackTimer>0 Then p\LightAttackTimer=p\LightAttackTimer-timervalue#
	If p\DashpadSpinTimer>0 Then p\DashpadSpinTimer=p\DashpadSpinTimer-timervalue#
	If p\AroundEnemyTimer>0 Then p\AroundEnemyTimer=p\AroundEnemyTimer-timervalue#
	If p\JumpPanelTimer>0 Then p\JumpPanelTimer=p\JumpPanelTimer-timervalue#
	If p\EnemyLeaderTimer>0 Then p\EnemyLeaderTimer=p\EnemyLeaderTimer-timervalue#
	If p\PanelStayTimer>0 Then p\PanelStayTimer=p\PanelStayTimer-timervalue#
	If p\Motion\Ground=False Then p\BeenInTheAirTimer=p\BeenInTheAirTimer+timervalue#
	If p\WaterrunTimer>0 Then p\WaterrunTimer=p\WaterrunTimer-timervalue#
	...
	If p\ClimbJumpTimer>0 Then p\ClimbJumpTimer=p\ClimbJumpTimer-timervalue#
	If p\Hurt2Timer>0 Then p\Hurt2Timer=p\Hurt2Timer-timervalue#
	...
	If p\RayAnimTimer>0 Then p\RayAnimTimer=p\RayAnimTimer-timervalue#
	If p\RocketBombTimer>0 Then p\RocketBombTimer=p\RocketBombTimer-timervalue#
	If p\Action=ACTION_UPPERCUT Then 
		p\UpperCutTimer=p\UpperCutTimer+timervalue#*1.5
		
		If p\UpperCutTimer>3*secs# Then 
			If p\UppercutSound=0  And Input\Hold\ActionSkill2 Then EmitSmartSound(Sound_Charged,p\Objects\Entity) : p\UppercutSound=1
			p\UpperCutTimer=3*secs#
			
		EndIf
	EndIf
	
	If p\GrindAffectorTimer>0 Then p\GrindAffectorTimer=p\GrindAffectorTimer-timervalue#
	
	If p\Action=ACTION_DEMODASH Then 
		p\DemoDashTimer=p\DemoDashTimer+timervalue#*2
		If p\DemoDashTimer>2.75*secs# Then 
			If p\UppercutSound=0 And Input\Hold\ActionSkill3 Then EmitSmartSound(Sound_Charged,p\Objects\Entity) : p\UppercutSound=1
			p\DemoDashTimer=2.75*secs#
		EndIf
		
		
	EndIf
	If p\ShootShotTimer>0 Then p\ShootShotTimer=p\ShootShotTimer-timervalue#
