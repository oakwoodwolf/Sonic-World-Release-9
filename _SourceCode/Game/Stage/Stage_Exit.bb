...
Function Game_Stage_End_Objects_ObjectIndividual(o.tObject)
	If o\ThisIsAnEnemy=True Then
		...
		If Menu\Settings\Shadows# = 2 Then FreeEntity(o\Enemy\ShadowCircle)
	EndIf
	...
	If o\HasDebugCube Then FreeEntity(o\DebugCube)
	
	If o\ObjType=OBJTYPE_TROPICAL Then
		For i=1 To 4 : FreeEntity o\ChaoObj\FruitMesh[i] : Next
	EndIf
	
	If o\HasVoiceClip Then 
		For i = 1 To 3 : FreeSound(o\VoiceClip[i]) : Next 
	EndIf
	...
	If o\HasValuesetTrigger Then Delete o\Trigger
	Delete o
End Function
...
Function Game_Stage_End_Objects()
	...
	For ch.tCheese = Each tCheese
		If Menu\Settings\Shadows# = 2 Then FreeEntity(ch\ShadowCircle)
	...
	For f.tFroggy = Each tFroggy
		If Menu\Settings\Shadows# = 2 Then FreeEntity(f\ShadowCircle)
...
Function Game_Stage_End_Player()
	For p.tPlayer = Each tPlayer
		...
		If Menu\Settings\Shadows#=2 And (Menu\ChaoGarden=0 Or Menu\Stage=999) Then FreeEntity(p\Objects\ShadowCircle)
		...
		FreeEntity p\Objects\RealJumpBall
		FreeEntity p\Objects\BoostBarrier
		FreeEntity p\Objects\PhantomBarrier
		FreeEntity p\Objects\Stomp
		FreeEntity p\Objects\Scanner
		FreeEntity p\Objects\OmoLap
		FreeEntity p\Objects\WaterBegg
...
Function Game_Stage_End_Chao()
	For cc.tChaoManager = Each tChaoManager
		If Menu\Settings\Shadows# = 2 Then FreeEntity(cc\ShadowCircle)
	...
	Next
End Function
Function Game_Stage_End_Mesh()
	For m.MeshStructure = Each MeshStructure
		FreeEntity m\Entity
		If m\Pivot<>0 Then FreeEntity m\Pivot
		If m\DestinationTarget<>0 Then FreeEntity m\DestinationTarget
		Select m\AnimTexType
			Case 1: FreeTexture m\DiffuseTexture[1]
			Case 2: For i=1 To 8 : FreeTexture m\DiffuseTexture[i] : Next
		End Select
		Delete m
	Next
End Function
...
Function Game_Stage_End()
	...
	StopChannel(Game\Stage\Properties\MusicStartChannel)
	...
	For x = 0 To MESHES_TOTAL 
		If MESHES_MODDED(x)=True Then
			If x=Mesh_DashPanelPads Then 
				FreeTexture Object_Texture_Pads
				Object_Texture_Pads=LoadTexture("Objects/Fasteners/Pad.png", 1+256)
			EndIf
			If x=Mesh_DashRampPads Then 
				FreeTexture Object_Texture_Panel
				Object_Texture_Panel=LoadTexture("Objects/Fasteners/Panel.png", 1+256)
			EndIf
		EndIf
		MESHES_MODDED(x)=False
	Next
...
Function Game_Stage_Restart()
	...
	If INTERFACE_EXISTS(Interface_BackgroundLoading) Then FreeSmartImage(Interface_BackgroundLoading)
...
Function Game_Stage_ReallyQuit(mode)
	...
	Select mode
		...
		Case 2: ;restart
			...
			Menu\ExitedAStage=0
		Case 3: ;exit
			If Menu\CollectionRoom=1 Then SaveGame_Inventory()
			Menu\Stage=0
			...
			ElseIf Menu\ChaoGarden=1 Or Menu\CollectionRoom=1 Or Menu\TutorialMode=1 Then 
			...
			Select 1
				Case Menu\ChaoGarden: Menu\NewOption=2 : Menu\ChaoGarden=0
				Case Menu\MarathonMode: Menu\NewOption=3 : Menu\MarathonMode=0
				Case Menu\CollectionRoom: Menu\NewOption=4 : Menu\CollectionRoom=0
				Case Menu\TutorialMode: Menu\NewOption=1 : Menu\TutorialMode=0
			End Select
		...
		Case 4: ;result quit
			If Menu\EmblemsGot=0 And Menu\AllRedRings=0 Then
			...
			Else
				...
				If Menu\AllRedRings=1 And Menu\EmblemsGot=0 Then
		...
		Case 5: ;timeover
			...
			Menu\NewMenu=MENU_GAMEOVER#
			Menu\NewOption=0
			If Menu\MissionCard=1 Then
				Menu\MissionCard=0
				Menu\GoBackToCollection=1
			EndIf
	...
	End Select
	Select Menu\NewMenu
		Case MENU_EMBLEM#,MENU_REDRING#,MENU_BLACKMARKET#,MENU_TRANSPORTER#,MENU_PRINCIPAL#,MENU_GAMEOVER#: Menu\Menu=Menu\NewMenu
		Default: Menu\Menu=0
	End Select
	Menu\Menu2=0
	Select mode
...
Function PauseAllChannels()
	...
	PauseChannel(Game\Stage\Properties\MusicStartChannel)
...
Function ResumeAllChannels()
	...
	ResumeChannel(Game\Stage\Properties\MusicStartChannel)
