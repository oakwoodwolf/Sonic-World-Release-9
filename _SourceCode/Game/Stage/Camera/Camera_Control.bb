...
Const CAMERA_CONTROL_AIRPITCH#=11
...
Function Camera_Control(c.tCamera,d.tDeltaTime)
	...
	If Game\CamLock>0 Or Game\CamLock2>0 Then
		...
		If c\FieldOfView#<c\Lock\Fov# Then c\FieldOfView#=c\FieldOfView#+c\Lock\RealSpeed#*d\Delta
		If c\FieldOfView#>c\Lock\Fov# Then c\FieldOfView#=c\FieldOfView#-c\Lock\RealSpeed#*d\Delta
		EndIf
	...
	ElseIf Game\Victory=0 Then
		...
		If Game\Interface\DebugPlacerOn=0 And Game\CinemaMode=0 Then
			If c\Target\Action=ACTION_DRIFT Or c\Target\Action=ACTION_BOARDDRIFT Or c\Target\Action=ACTION_CARDRIFT Then
				If c\Target\DriftDirection=1 Then c\TargetRotation\y#=c\TargetRotation\y#-((0.5*c\Target\HoldingDrift)*c\Target\DriftTimer/secs#)*d\Delta
				If c\Target\DriftDirection=-1 Then c\TargetRotation\y#=c\TargetRotation\y#+((0.5*c\Target\HoldingDrift)*c\Target\DriftTimer/secs#)*d\Delta
			ElseIf Input\Pressed\CamCenter Then
				If (Not(Game\CamLock>0)) Then c\TargetRotation\y#=c\Target\Animation\Direction#-180
		...
		If (Not(c\MouseCameraTimer>0))  Then
			If c\Target\Motion\Ground=False And (Not(c\Target\Action=ACTION_BUOY Or c\Target\Action=ACTION_TORNADO Or c\Target\Action=ACTION_STOMP Or c\Target\BoardWaterTimer>0)) And c\Target\BeenInTheAirTimer>2.5*secs# And Game\MachLock=0 Then
		...
		If Input\Hold\MouseCamZoomOut Then
			If c\Target\Action=ACTION_DEBUG Then
				If CAMERA_ACTUALZOOMVALUE#<500 Then CAMERA_ACTUALZOOMVALUE#=CAMERA_ACTUALZOOMVALUE#+2.5*d\Delta
			Else
				If CAMERA_ACTUALZOOMVALUE#<250 Then CAMERA_ACTUALZOOMVALUE#=CAMERA_ACTUALZOOMVALUE#+2.5*d\Delta
			EndIf
		EndIf
	...
	If Game\RunLock>0 Or Game\TwoDLock=1 Or Game\CinemaMode>0 Then c\Mode=CAMERA_MODE_NORMAL Else c\Mode=CAMERA_MODE_TARGETPOV
