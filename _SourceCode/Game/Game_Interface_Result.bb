...
;		Game\Rank = 1
		;Floor(0.3*Interface_Render_Result_GetRank_Score(p)+0.7*Interface_Render_Result_GetRank_Time(p))
	
	If Menu\RankMissionOnTime=0 Then 
		If Game\ScoreTotal>=Game\IdealScore Then
			Game\Rank=1
		ElseIf Game\ScoreTotal>=Game\IdealScore*0.95 Then
			Game\Rank=2
		ElseIf Game\ScoreTotal>=Game\IdealScore*0.85 Then
			Game\Rank=3
		ElseIf Game\ScoreTotal>=Game\IdealScore*0.7 Then
			Game\Rank=4
		ElseIf Game\ScoreTotal>=Game\IdealScore*0.6 Then
			Game\Rank=5
		ElseIf Game\ScoreTotal>=Game\IdealScore*0.5 Then
			Game\Rank=6
		ElseIf Game\ScoreTotal<Game\IdealScore*0.5 Then
			Game\Rank=7
		Else
	...
	Else
		If Game\ResultTime<=Game\IdealTime Then
			rn=1
		ElseIf Game\ResultTime>Game\IdealTime+75*secs# Then
			rn=7
		ElseIf Game\ResultTime>Game\IdealTime+60*secs# Then
			rn=6	
		ElseIf Game\ResultTime>Game\IdealTime+45*secs# Then
			rn=5
		ElseIf Game\ResultTime>Game\IdealTime+30*secs# Then
			rn=4	
		ElseIf Game\ResultTime>Game\IdealTime+15*secs# Then
			rn=3	
		ElseIf Game\ResultTime>Game\IdealTime Then
			rn=2	
		EndIf
		Game\Rank=rn
	EndIf 
	...
	If Game\Cheater=0 And Menu\TutorialMode=0 And Menu\MarathonMode=0 And Menu\HubStage<>9999 And Menu\Stage<=NonModStageAmount Then 
		...
		RANK(Menu\MissionNo,Menu\Stage)=Game\Rank
		
		If Menu_AllEncoresBeaten() And StageLocked(NonModStageAmount)=1 Then
			StageLocked(NonModStageAmount)=0
			Achievement_Grant("Beat all Encore acts! Worst Cave unlocked.",2,500,Sound_RingSuper)
		EndIf
...
Function Menu_AllEncoresBeaten()
	
	i=0
	j=0
	Repeat
		i=i+1 
		If EMBLEMS1(5,i)=1 Then j=j+1
	Until i=NonModStageAmount-1
	
	If j=NonModStageAmount-1 Then Return True Else Return False
...
End Function
	...
	Function Interface_Render_Result_Exit(p.tPlayer)
		...
		If Menu\TutorialMode=1 Then Menu\TutorialMode=0
...
Function Interface_Render_Result_Save(p.tPlayer)
	...
	If Menu\ChaoGarden=0 And Menu\MissionCard=0 And Menu\TutorialMode = 0 Then
		LoadGame_Records(Menu\StageName$)
		...
		If Game\Cheater=0 Then;!!!
			...
			SaveGame_Records(Menu\StageName$)
			
			If Menu\Stage<=NonModStageAmount Then
				SaveGame_Inventory()
				...
				TOKENS=TOKENS+Game\TokensToAdd
				...
				If Menu\MissionNo=1 Then
					For i=1 To 5
						If Game\Gameplay\GotRedRing[i]=1 And REDRING(i,Menu\Stage)=0 Then REDRING(i,Menu\Stage)=1 : REDRINGS=REDRINGS+1
					Next
					If ALLREDRING(Menu\Stage)=0 Then
						If REDRING(1,Menu\Stage)=1 And REDRING(2,Menu\Stage)=1  And REDRING(3,Menu\Stage)=1  And REDRING(4,Menu\Stage)=1  And REDRING(5,Menu\Stage)=1 Then ALLREDRING(Menu\Stage)=1 : Menu\AllRedRings=1
					EndIf
				EndIf
				
				EMBLEMS=EMBLEMS+Menu\EmblemsGot
				
				
				
				
				
				SaveGame_Emblems(Menu\StageName$)
			EndIf
		Else;!!!
			Menu\EmblemsGot=0
		EndIf;!!!
...
End Function
	...
	End Function
	Dim ABILITY_ADDEDTORESULT(SHOPITEM_TOTAL(SHOPMENU_ABILITY))
	Dim HANDICAP_ADDEDTORESULT(SHOPITEM_TOTAL(SHOPMENU_HANDICAP))
	; ---------------------------------------------------------------------------------------------------------
	; ---------------------------------------------------------------------------------------------------------
...
Function Interface_Render_Result_Stage(p.tPlayer,d.tDeltaTime)
	...
	If Menu\ThemeVictoryTimer>0 Then
		If Game\ReadyToSeeVictoryTimer>0 Then 
			Game\ReadyToSeeVictoryTimer=Game\ReadyToSeeVictoryTimer-timervalue#
			If (Not(Game\ReadyToSeeVictoryTimer>0)) Then Game\ReadyToSeeVictory=1
		EndIf
	EndIf
	
	If Menu\ActiveAbilities>0 Then
		For i = 1 To Menu\ActiveAbilities
			DrawImageEx(INTERFACE((Interface_Icons_Ability)), (35*i)*GAME_WINDOW_SCALE#, (10+25)*GAME_WINDOW_SCALE#, Game\Interface\AbilityIcon[i])
		Next
	EndIf
	
	If Menu\ActiveHandicaps>0 Then 
		If Menu\ActiveAbilities>0 Then h=70 Else h=35
		For i = 1 To Menu\ActiveHandicaps
			DrawImageEx(INTERFACE((Interface_Icons_Handicap)), (35*i)*GAME_WINDOW_SCALE#, (h)*GAME_WINDOW_SCALE#, Game\Interface\HandicapIcon[i])
		Next
	EndIf
	...
	If Game\Interface\ResultOrder<7 Then ;!
		...
		If Game\Interface\ResultOrder>=2 Then
			...
			If Game\Interface\ResultOrder>2 Or Game\Interface\ResultOrder2>=4 Then
				DrawRealText("BONUS", GAME_WINDOW_W/2-161*GAME_WINDOW_SCALE#, GAME_WINDOW_H/2+29.5*GAME_WINDOW_SCALE#, (Interface_TextControls_2))
			...
			If Game\Interface\ResultOrder>2 Or Game\Interface\ResultOrder2>=5 Then
				DrawRealText("TOTAL", GAME_WINDOW_W/2-161*GAME_WINDOW_SCALE#, GAME_WINDOW_H/2+54*GAME_WINDOW_SCALE#, (Interface_TextControls_2))
		...
		If Game\Interface\ResultOrder>=4 Then
			...
			If Game\Interface\ResultOrder>4 Or Game\Interface\ResultOrder2>1 Then
				If Game\Rank>0 Then 
					...
					Rank_Draw(Game\Rank, GAME_WINDOW_W/2+36.295*GAME_WINDOW_SCALE#, GAME_WINDOW_H/2+103.0*GAME_WINDOW_SCALE#)
	...
	Select Game\Interface\ResultOrder
		Case 0: ; MISSION CLEAR
			If (Not(ChannelPlaying(Game\Channel_MissionCompleted))) Or Game\ReadyToSeeVictory=1 Or Game\Interface\ResultOrder2=-1 Then
				Select Game\Interface\ResultOrder2
					Case 0:
						Game\Interface\ResultTitlePosition#=0
			...
			Select Game\Interface\ResultOrder2
				Case 0,-1:
					If (Not(Game\Interface\ResultTimer>0)) Or Game\Interface\ResultOrder2=-1 Then
						If Menu\ThemeContinuousVictory=0 Then
							If Game\Rank=1 Then
								Game\Channel_Result=PlaySmartSound(Sound_ResultS)
							Else
								Game\Channel_Result=PlaySmartSound(Sound_Result)
							EndIf
							ChannelVolume(Game\Channel_Result,Menu\Settings\VolumeM#*(Menu\Settings\Volume#*0.175))
						EndIf
	...
	If Game\Cheater=1 And Menu\ChaoGarden=0 Then
		DrawRealText("You didn't play fair and now", GAME_WINDOW_W/2, GAME_WINDOW_H/2-150*GAME_WINDOW_SCALE#, (Interface_TextControls_2), 1)
		DrawRealText("you will pay the price. No records for you.", GAME_WINDOW_W/2, GAME_WINDOW_H/2+150*GAME_WINDOW_SCALE#, (Interface_TextControls_2), 1)
	EndIf
	
	If Menu\Settings\CONTROLTIPS#=1 Then
		Select Game\Interface\ResultOrder
			Case 0,1,2,3,4,5,7: Interface_ControlTipDraw_Button("Skip", INPUT_BUTTON_ACTIONJUMP, GAME_WINDOW_W-20*GAME_WINDOW_SCALE#, GAME_WINDOW_H-20*GAME_WINDOW_SCALE#)
			Case 6: Interface_ControlTipDraw_Button("Continue", INPUT_BUTTON_ACTIONJUMP, GAME_WINDOW_W-20*GAME_WINDOW_SCALE#, GAME_WINDOW_H-20*GAME_WINDOW_SCALE#)
		End Select
	EndIf
