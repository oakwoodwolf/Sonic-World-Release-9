...
Function AboutToChat()
	...
	If key
		If key=13 And Chatting\Txt$<>"" then
			...
			Else ; Chat Commands
				...
				select kicktxt$
					...
					case "/wizg", "/WizG", "/WIZG" : kicktxt$=kicktxt$+Chr$(32) : Info("ALL HAIL WIZG!!!", 255,255,255, "bold", true) : BP_UDPMessage(0,UDPMSG_MESSAGE, "ALL HAIL WIZG!!!")
					case "/420", "/blazeit" : kicktxt$=kicktxt$+Chr$(32) : Info("420 Blaze It!!!!", 255,255,255, "bold", true) : BP_UDPMessage(0,UDPMSG_MESSAGE, "420 Blaze It!!!!")
		...
		ElseIf key>=32 And key<127 then ;typing
			if Len(Chatting\Txt$)<63 Then Chatting\Txt$=Chatting\Txt$+Chr$(key) : PlaySmartSound(Sound_MenuMove)
		EndIf
	EndIf
End Function
...
Function Interface_DrawChat(x#=0, y#=0, csize=1, orientation=1)
	...
	if slidechat#>=18 then slidechat#=18
	textcounter = textcounter - slidechat#
